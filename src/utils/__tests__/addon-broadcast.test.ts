...
/**
 * Tests for the cross-tab addon broadcast utility.
 *
...
import {
  broadcastAddonChange,
  onAddonChange,
  type AddonChangeMessage,
} from '../addon-broadcast';

describe('addon-broadcast', () => {
  ...
  it('broadcastAddonChange does not throw even without BroadcastChannel', () => {
    expect(() => broadcastAddonChange('item-1', 'rest-1')).not.toThrow();
  });

  it('onAddonChange returns an unsubscribe function', () => {
    const unsub = onAddonChange(() => {});
  ...
  if (hasBroadcastChannel) {
    it('listener receives messages broadcast on the same channel', async () => {
      const received: AddonChangeMessage[] = [];
      const unsub = onAddonChange((msg) => received.push(msg));

      broadcastAddonChange('item-42', 'rest-99');
  ...
  it('unsubscribe prevents further callbacks', async () => {
    const cb = vi.fn();
    const unsub = onAddonChange(cb);
    unsub();

    broadcastAddonChange('item-1', 'rest-1');
    await new Promise((r) => setTimeout(r, 50));
    vi.advanceTimersByTime(50);

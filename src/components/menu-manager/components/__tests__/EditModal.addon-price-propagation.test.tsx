...
// ── 5. AddonCard display — reads from addonPool, not price_override ──────────

describe('EditModal — AddonCard shows price from addonPool (read-only)', () => {
  it('shows the addon pool price, ignoring a stale price_override on the association', async () => {
    /**
     * Design rule: addon price is always authoritative on the addon item itself.
     * price_override on the association is legacy/write-path only and must never
     * be used for display.
     *
     * Scenario: dish has an addon associated with price_override: 0 (stale).
     * getAddonItems returns the addon item with price: 2.50.
     * The AddonCard must show $2.50, not $0.00.
     */
    const user = userEvent.setup();

    const addonEntry: AddonEntry = {
      menu_item_id: 'addon-1',
      name: 'Extra Chutney',
      price_override: 0, // stale — pre-fix data
      thumbnail_url: null,
      status: 'approved',
      suggestion_source: 'manual',
    };
    const dishItem = makeDishItem('dish-1', { addons: [addonEntry] });
    const addonItemInPool = makeAddonItem({ id: 'addon-1', name: 'Extra Chutney', price: 2.5 });

    const service = makeService({
      getAddonItems: vi.fn().mockResolvedValue([addonItemInPool]),
    });
    ...
    // Navigate to Addons tab — this triggers getAddonItems fetch
    await user.click(screen.getByTestId('tab-addons'));

    // Wait for addonPool to load and price to appear
    await waitFor(() => {
      const priceEl = document.querySelector('[data-testid="addon-price-addon-1"]');
      expect(priceEl).not.toBeNull();
      expect(priceEl!.textContent).toBe('$2.50');
    });
  });

  it('shows empty price string when the addon is not in the loaded pool', async () => {
    /**
     * Edge case: addon is associated with the dish but getAddonItems returns an
     * empty pool (or the specific addon item has been deleted). The AddonCard must
     * show no price — an empty string — not $0.00 or an error.
     */
    const user = userEvent.setup();

    const addonEntry: AddonEntry = {
      menu_item_id: 'addon-orphan',
      name: 'Deleted Addon',
      price_override: 5,
      thumbnail_url: null,
      status: 'approved',
    ...
    const service = makeService({
      // Pool is empty — addon item was deleted from the system
      getAddonItems: vi.fn().mockResolvedValue([]),
    });
    ...
    await waitFor(() => {
      const priceEl = document.querySelector('[data-testid="addon-price-addon-orphan"]');
      expect(priceEl).not.toBeNull();
      // basePrice is null (not in pool) → renders as empty string, not $5.00
      expect(priceEl!.textContent).toBe('');
    });
  });

  it('has no editable price input for the addon association (price is read-only)', async () => {
    /**
     * The old design had an editable <input> next to each addon in the card.
     * After the design correction, there is no input — price is purely display.
     * Asserting this prevents regressions that re-introduce per-association editing.
     */
    const user = userEvent.setup();
    ...
    // Wait for addon card to render
    await waitFor(() => {
      expect(document.querySelector('[data-testid="remove-addon-modal-addon-1"]')).toBeInTheDocument();
    });

    // The price element is a <div>, not an <input> — no editable price field
    const priceEl = document.querySelector('[data-testid="addon-price-addon-1"]');
    expect(priceEl).not.toBeNull();
    expect(priceEl!.tagName.toLowerCase()).not.toBe('input');

    // No input elements inside the AddonCard price area
    expect(priceEl!.querySelector('input')).toBeNull();
  });
});

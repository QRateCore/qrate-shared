...
// PDD 2026-05-22 Step 7: workflow simplifies to
//   Order Placed → Entered in POS → Served
// `ready` is retained in OrderStatus + STATUS_DISPLAY here as a
// legacy-row safety net (any pre-existing rows with status='ready'
// from before the Step 6b backfill still render with a sensible
// label). The waiter app now writes `delivered` directly from the
// In Kitchen group; `ready` is no longer an active outbound state.
// Final cleanup (removing `ready` from the union + STATUS_DISPLAY +
// ACTIVE_STATUSES) lands in Step 6c after the backfill confirms
// zero legacy rows.
export const STATUS_DISPLAY: Record<string, { label: string; bg: string; text: string; ringColor: string }> = {
  ...
  ready:     { label: 'Served',         bg: 'bg-emerald-50', text: 'text-emerald-800', ringColor: 'ring-emerald-200' },
...
// `ready` → `delivered` retained for any legacy rows still in the DB
// (Step 6b backfill will eliminate them; Step 6c drops this case).
...
// `ready` retained for legacy-row visibility until Step 6c.
